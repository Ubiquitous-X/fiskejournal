...
export default {
  ...
  setup() {
    ...
    const formattedDateDisplay = (date) => {
      ...
      // Skapa den formaterade strängen och använd capitalizeFirstLetter för att kapitalisera veckodagen
      const formattedDate = new Date(date).toLocaleDateString('sv-SE', options);
      const [weekday, rest] = formattedDate.split(', '); // Dela upp strängen i veckodag och resten
      return `${capitalizeFirstLetter(weekday)}`; // Sätt ihop strängen igen med kapitaliserad veckodag
    };
    ...
    // Funktion för att översätta vindriktningen
    const translateWindDir = (dir) => {
      // Ersätt "W" med "V" och "E" med "Ö"
